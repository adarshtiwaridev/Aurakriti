...
import { addToWishlist, removeFromWishlist } from '@/redux/slices/wishlistSlice'; // ✅ added
...
export default function ProductCard({ product, onAddToCart }) {
  ...
  const wishlistItems = useSelector((state) => state.wishlist.items); // ✅ added
  ...
  const safeImage =
    product.image ||
    product.images?.[0] ||
    'https://via.placeholder.com/800x800?text=Jewellery';
  ...
  return (
    <>
        ...
        <div className="flex flex-1 flex-col p-6">
          <div className="mb-2 flex items-center justify-between gap-3">
            <Link
              ...
              className="text-lg font-semibold text-[#3c2f25] transition hover:text-[#c9a14a]"
            >
              {safeName}
            </Link>
            <p className="text-lg font-black text-[#c9a14a]">
              Rs {Number(product.price || 0).toLocaleString('en-IN')}
            </p>
          </div>

          <p className="mb-4 text-xs font-semibold uppercase tracking-[0.2em] text-[#967449]">
            Rating {rating}/5
          </p>

          <p className="line-clamp-2 text-sm leading-6 text-[#6b5546]">
            {product.description}
          </p>
      ...
      {showQuickView ? (
        <div
          className="fixed inset-0 z-60 flex items-center justify-center bg-[#2c241d]/35 p-4"
          onClick={() => setShowQuickView(false)}
        >
          <div
            className="grid w-full max-w-3xl gap-5 rounded-3xl bg-white p-5 shadow-2xl md:grid-cols-[0.95fr_1.05fr]"
          ...
          >
            ...
            <div className="flex flex-col">
              ...
              <h3 className="mt-2 text-2xl font-black text-[#3a2f24]">
                {safeName}
              </h3>

              <p className="mt-2 text-sm leading-6 text-[#6b5546]">
                {product.description}
              </p>

              <p className="mt-3 text-2xl font-black text-[#c9a14a]">
                Rs {Number(product.price || 0).toLocaleString('en-IN')}
              </p>
              ...
              <div className="mt-auto flex flex-wrap gap-2 pt-5">
                ...
                <Link
                  href={`/products/${productId}`}
                  className="rounded-xl border border-[#e8dccb] px-4 py-2 text-sm font-semibold text-[#5f4b38]"
                >
                  View Product
                </Link>
